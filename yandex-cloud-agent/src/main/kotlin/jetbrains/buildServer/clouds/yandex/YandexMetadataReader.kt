...
class YandexMetadataReader(events: EventDispatcher<AgentLifeCycleListener>,
                           private val configuration: BuildAgentConfigurationEx,
                           private val idleShutdown: IdleShutdown) {

    init {
        LOG.info("Yandex plugin initializing...")
    ...
    private fun updateConfiguration(json: String) {
        ...
        if (metadata == null) {
            LOG.info("Metadata Compute integration is not available: Invalid instance metadata")
            LOG.debug(json)
            return
        }
        ...
        if (data == null) {
            LOG.info("Metadata Compute integration is not available: No TeamCity metadata")
            LOG.debug(json)
            return
        }

        LOG.info("Metadata Compute integration is available, will register agent \"${metadata.name}\" on server URL \"${data.serverAddress}\"")
        ...
        configuration.addConfigurationParameter(YandexAgentProperties.INSTANCE_NAME, metadata.name)
    ...
    companion object {
        private val LOG = Logger.getInstance(YandexMetadataReader::class.java.name)
        ...
        private const val ERROR_GCE_UNAVAILABLE = "Metadata Compute integration is not available: "
