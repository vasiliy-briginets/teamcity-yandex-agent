...
class IdleShutdown(private val myTracker: AgentIdleTimeTracker,
    ...
    private val myService = ExecutorsFactory.newFixedScheduledDaemonExecutor("Yandex instance shutdown on idle time", 1)
    ...
    private val shutdownCommands: Array<String>
        get() {
            ...
            if (info.isWindows) {
                return arrayOf("shutdown -s -t 1 -c \"TeamCity Yandex Agent Instance shutdown on idle time\" -f", "shutdown /s /t 1 /c \"TeamCity Yandex Agent Instance shutdown on idle time\" /f")
            }
