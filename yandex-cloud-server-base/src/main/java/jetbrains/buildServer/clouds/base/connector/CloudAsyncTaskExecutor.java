...
public class CloudAsyncTaskExecutor {
    ...
    private final Map<AsyncCloudTask, Long> myLongTasks = new HashMap<>();

    public CloudAsyncTaskExecutor(String prefix) {
        myExecutingTasks = new ConcurrentHashMap<>();
        myExecutor = ExecutorsFactory.newFixedScheduledDaemonExecutor(prefix, 2);
        scheduleWithFixedDelay("Check for tasks", this::checkTasks, 0, 300, TimeUnit.MILLISECONDS);
    }

    public void scheduleWithFixedDelay(@NotNull final String taskName, @NotNull final Runnable task, final long initialDelay, final long delay, final TimeUnit unit) {
        myExecutor.scheduleWithFixedDelay(() -> NamedThreadFactory.executeWithNewThreadName(taskName, task), initialDelay, delay, unit);
    }

    public void submit(final String taskName, final Runnable r) {
        myExecutor.submit(() -> {
            try {
