  ...
  public void updateErrors(@Nullable final TypedCloudErrorInfo... errors){
    ...
    if (errors != null && errorInfoMap.size() > 0) {
      if (errorInfoMap.size() == 1) {
        ...
        final String details = err.getDetails();
        if (err.getThrowable() != null) {
          myErrorInfo.set(new CloudErrorInfo(message, details, err.getThrowable()));
        } else {
          myErrorInfo.set(new CloudErrorInfo(message, details));
        }
      } else {
