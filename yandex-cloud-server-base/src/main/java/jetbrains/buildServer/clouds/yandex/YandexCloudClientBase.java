...
/**
 * Yandex cloud client base.
 */
public abstract class YandexCloudClientBase<G extends AbstractCloudInstance<T>, T extends AbstractCloudImage<G, D>, D extends CloudImageDetails>
  extends AbstractCloudClient<G, T, D> {

  private static final Logger LOG = Logger.getInstance(YandexCloudClientBase.class.getName());
  private final YandexCloudImagesHolder myImagesHolder;

  public YandexCloudClientBase(@NotNull final CloudClientParameters params,
                               @NotNull final CloudApiConnector<T, G> apiConnector,
                               @NotNull final YandexCloudImagesHolder imagesHolder) {
  ...
  public G findInstanceByAgent(@NotNull final AgentDescription agent) {
    final String instanceName = agent.getConfigurationParameters().get(YandexAgentProperties.INSTANCE_NAME);
  ...
  public String generateAgentName(@NotNull final AgentDescription agent) {
    final String instanceName = agent.getConfigurationParameters().get(YandexAgentProperties.INSTANCE_NAME);
    LOG.debug("Reported yandex instance name: " + instanceName);
    return instanceName;
  }
