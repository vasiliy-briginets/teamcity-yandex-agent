 ...
 */
public class YandexCloudImagesHolder {
  private final Map<String, AbstractCloudImage> myImages = new ConcurrentHashMap<>();

  @Nullable
  AbstractCloudImage findImage(@NotNull final String profileId, @NotNull final String sourceId) {
    final String key = getKey(profileId, sourceId);
    return myImages.get(key);
  }

  void addImage(@NotNull String profileId, @NotNull AbstractCloudImage image) {
